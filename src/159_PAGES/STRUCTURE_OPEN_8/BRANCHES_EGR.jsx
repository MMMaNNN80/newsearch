...
const BRANCHES_EGR = (props) => {
 ...
 const [perCountPages, setPerCountPages] = useState(5)
 ...
 let currentPages=[]
  ...
  if (!mainForm || !mainForm.massBranchesEgrul) { return null }
    ...
    <Fragment>
      ...
      {mainForm.massBranchesEgrul.length > 0 ?
        <>
          ...
          <div style={{width:"100%", backgroundColor:"lightblue" , opacity: 0.9,padding:0}}>
          <img style={{...styleimg}} src="..\..\icon\openLists.svg" height="15px" alt="иконка списка"
              onClick={onClickHandler }></img>
             <span style={{color: "blue" ,fontSize:"14px"}}> {`Филиалы по данным ЕГРЮЛ (количество - ${cnt})`} </span>
            </div>
