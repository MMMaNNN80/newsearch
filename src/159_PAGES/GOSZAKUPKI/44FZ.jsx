...
const FZ44 = ({ mainForm }) => {
    ...
    if (!mainForm.mass44FZAGG || mainForm.mass44FZAGG.length < 1) {
         return (getEmpty('Данные по участию в госзакупках по 44-ФЗ отсутствуют')) }
    ...
    mass.push(head)
    mainForm.mass44FZAGG.forEach(el => {
        let perTenders = '0'
        let perSumm = '0'
        ...
        mass.push
            ...
            <div className={{}} style={{ width: '60%' }}> {el.cnt}
                <div className={'graf_44'} style={{ ...styles, ...{ width: `${perTenders}%` } }}></div>
            </div>,
            <div className={{}} style={{ width: '80%' }}> {el.p_price}
                <div className={'graf_44'} style={{ ...styles, ...{ width: `${perSumm}%`, display: 'block' } }}>
                </div> </div>,
    ...
    return (
        <Fragment>
            <br />
            <ul style={{ color: 'lightblue', fontSize: '12px' }}>
                {mainForm.mass44FZAGG[0].is_supfz44 === 1 ?
                    <li> Имеются сведения об участии в госзакупках в роли Поставщика (ФЗ-44)</li> : ''}
                {mainForm.mass44FZAGG[0].is_supfz223 === 1 ?
                    <li>Имеются сведения об участии в госзакупках в роли Поставщика (ФЗ-223)</li> : ''}
            </ul>
            <GETTABLE funcGetRows={[...getMassRows(mass)]}  //Регистрационные данные
                ...
                name={"Данные о закупках в соответствии с 44-ФЗ (в роли Заказчика)"}
                endtbl={true}
