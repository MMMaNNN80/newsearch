...
const PAO_ROSSTAT_UL_COWNERS = ({massRosstatPAO}) => {
    ...
    let massCowsUL = [] //Физ лица
        ...
        let mass = []
        mass = massRosstatPAO.filter(el => el.coowner_type === 0)
        if (mass.length > 0) {
    ...
    return (
        <Fragment>
            {massCowsUL.length > 0 ? <GETTABLE funcGetRows={[...getMassRows(massCowsUL)]}  //Регистрационные данные
                ...
                name={"Юридические лица в структуре совладения (Росстат) ПАО"} 
                endtbl = {true}
                /> : 
...
export default PAO_ROSSTAT_UL_COWNERS;